...
local GOLD_SOFT_CAP_GOLD = math.floor(GOLD_SOFT_CAP_COPPER / 10000)
...
local function MarkAccountDirty(accountId)
...
local function RetroactivelySeedAccountMoneyIfMissing(accountId)
    CharDBExecute(string.format([[
        INSERT IGNORE INTO accountwide_money (accountId, money)
        SELECT %d, COALESCE(SUM(money), 0)
        FROM characters
        WHERE account = %d
    ]], accountId, accountId))
end
...
local function FlushMoneyDelta(player, accountId, guid, updateBaseline, forceFlush)
    local now = GetNowMs()
    local didPush = false

    if (not forceFlush) and lastFlushAt[guid] and (now - lastFlushAt[guid]) < FLUSH_DEBOUNCE_MS then
...
local function TickRealtimeSync(player)
    ...
    local didPush = FlushMoneyDelta(player, accountId, guid, true)
    ...
    if finalBaseline == accountTotal or not changed then
        accountDirty[accountId] = nil
        accountDirtyAt[accountId] = nil
    end
end
...
local function CleanUpMoneyOnLogout(event, player)
    ...
    FlushMoneyDelta(player, accountId, guid, false, true)
